/**
 * ATS Analyzer - Server-side version
 * Identical logic to client-side but optimized for Node.js
 */
...
export class ATSAnalyzer {
  constructor() {
    ...
    this.requiredSections = ['contact', 'email', 'phone', 'experience', 'work', 'education', 'skills'];
  }
  ...
  analyzeKeywords(text) {
    const found = { technical: 0, soft: 0, action: 0 };
    
    for (const [category, keywords] of Object.entries(this.keywords)) {
      keywords.forEach(keyword => {
        if (text.includes(keyword.toLowerCase())) found[category]++;
      });
    }
    ...
    return { score: Math.round(avgScore), found, total: this.keywords };
  }
  ...
  analyzeSections(text) {
    ...
    return { score: Math.round(score), found: foundSections, missing: missingSections };
  }
  ...
  calculateOverallScore(scores) {
    const weights = { keywords: 0.30, sections: 0.25, formatting: 0.20, length: 0.15, contact: 0.10 };
    let totalScore = 0;
  ...
  identifyIssues(scores, text) {
    ...
    if (scores.sections.missing.length > 0) {
      issues.push({
        ...
        description: `Consider adding: ${scores.sections.missing.slice(0, 2).join(', ')}.`
      });
    }
  ...
  generateRecommendations(scores, issues) {
    ...
    if (scores.keywords.score < 70) recommendations.push('Add more industry-specific keywords');
    if (scores.contact.score < 100) recommendations.push('Include complete contact information');
    if (scores.sections.missing.length > 0) recommendations.push('Add missing sections');
